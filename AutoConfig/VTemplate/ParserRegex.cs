...
namespace VTemplate.Engine
{
    ...
    internal static class ParserRegex
    {
        ...
        static ParserRegex()
        {
            TagRegex = new Regex(@"\G<vt\:\s*(?<tagname>[\-\w]+)(\s+(?<attrname>[\-\w]+)(\s*=\s*""(?<attrval>[^""]*)""|\s*=\s*'(?<attrval>[^']*)'|\s*=\s*(?<attrval>[^\s=>]*)|(?<attrval>\s*?)))*\s*(?<closed>/)?>", RegexOptions.IgnoreCase);

            EndTagRegex = new Regex(@"\G</vt\:\s*(?<tagname>[\-\w]+)(\s+(?<attrname>[\-\w]+)(\s*=\s*""(?<attrval>[^""]*)""|\s*=\s*'(?<attrval>[^']*)'|\s*=\s*(?<attrval>[^\s=>]*)|(?<attrval>\s*?)))*\s*>", RegexOptions.IgnoreCase);

            string varExp = @"(?:#(?<prefix>#|[\-\w]*)\.)?(?<name>(?!\d)\w+)(?:\.(?<field>\d+|(?!\d)\w+)(?<method>(?:\( *\))?))*";

            VarTagRegex = new Regex(@"\G{\$\:?\s*" + varExp + @"(\s+(?<attrname>[\-\w]+)(\s*=\s*""(?<attrval>[^""]*)""|\s*=\s*'(?<attrval>[^']*)'|\s*=\s*(?<attrval>[^\s=}]*)|(?<attrval>\s*?)))*\s*}", RegexOptions.IgnoreCase);

            VarExpRegex = new Regex(@"^\s*" + varExp + @"\s*$", RegexOptions.IgnoreCase);

            VarIdRegex = new Regex(@"^\s*\$?(?:#(?<prefix>#|[\-\w]*)\.)?(?<name>(?!\d)\w+)\s*$" );
        }
    }
}
