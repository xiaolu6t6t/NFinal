    ...
    public class Compiler
    {
        ...
        private string BuildGetParameterCode(string type,bool isArray, string name,bool hasDefaultValue,string defaultValue)
        {
            ...
            if (!isArray)
            {
                switch (type.ToLower())
                {
                    case "string": code = string.Format("{0} {1}=get[\"{1}\"]==null?{2}:get[\"{1}\"];", type, name,defaultValue); break;
        ...
        /// <returns></returns>
        public FileData GetFileData(string fileName, System.Text.Encoding encoding)
        {
                                ...
                                if(!string.IsNullOrEmpty(methodData.parameters))
                                {
                                    ...
                                    for (int k = 0; k < parameters.Length; k++)
                                    {
                                        ...
                                        if (mParameter.Success)
                                        {
                                            ...
                                            parameterData.hasDefaultValue = mParameter.Groups[4].Success;
                                            parameterData.defaultValue = mParameter.Groups[4].Value;
