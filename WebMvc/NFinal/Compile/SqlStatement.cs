...
namespace NFinal.Compile
{
    ...
    public class SqlStatement
    {
        ...
        public static Function[] AggregateFunctions = {
            new Function{ name="AVG",returnType="int",parametersCount=1,parameterPosition=1},
            new Function{ name="COUNT",returnType="int",parametersCount=1,parameterPosition=1},
            new Function{ name="FIRST",returnType=null,parametersCount=1,parameterPosition=1},
            new Function{ name="LAST",returnType=null,parametersCount=1,parameterPosition=1},
            new Function{ name="MAX",returnType="int",parametersCount=1,parameterPosition=1},
            new Function{ name="MIN",returnType="int",parametersCount=1,parameterPosition=1},
            new Function{ name="SUM",returnType="int",parametersCount=1,parameterPosition=1}
            };
        /// <summary>
        /// SQL中的函数类
        /// </summary>
        ...
        public List<SqlVarParameter> ParseVarName(string sql)
        {
            ...
            if (mac.Count > 0)
            {
                for (int i = 0; i < mac.Count; i++)
                {
                    ...
                    if (mat.Success)
                    {
                        par = new SqlVarParameter();
                        par.sql = mat.Value;
                        par.columnName = mat.Groups[1].Value;
                        par.name = mat.Groups[2].Value;
                        parameters.Add(par);
                    }
                }
            }
            ...
            if (mac.Count > 0)
            {
                for (int i = 0; i < mac.Count; i++)
                {
                    ...
                    if (mat.Success)
                    {
                        ...
                        par.sql = mat.Value;
                        par.name = mat.Groups[1].Value;
                        par.columnName = mat.Groups[2].Value;
                        parameters.Add(par);
                    }
                }
            }
            ...
            string fieldLikeParameter =
                @"([_a-zA-z][_a-zA-Z0-9]*)\s+like\s+(\S+(?:\s*\+\s*\S+)*)";
            ...
            if (mac.Count > 0)
            {
                for (int i = 0; i < mac.Count; i++)
                {
                    ...
                    if (mat.Success)
                    {
                        varNameString = mat.Groups[2].Value;
            ...
            string fieldBetweenParameter =
                @"([_a-zA-z][_a-zA-Z0-9]*)\s+between\s+(\S+)\s+and\s+(\S+)";
            ...
            if (mac.Count > 0)
            {
                for (int i = 0; i < mac.Count; i++)
                {
                    ...
                    if (mat.Success)
                    {
                        ...
                        varNameString = mat.Groups[3].Value;
                        varNameMat = varNameReg.Match(varNameString);
                        if (varNameMat.Success)
                        {
                            par = new SqlVarParameter();
