...
namespace NFinal.Compile
{
    ...
    {
        public static Regex viewReg = new Regex(@"View\s*\((?:\s*(\S+),)?\s*(?:""([^""]*)"")?\s*\)\s*;[^\r\n]*[\r\n]*");
        /// <summary>
        /// 在某段位置替换成另一段代码
        /// </summary>
        ...
        /// <returns></returns>
        public List<ViewData> Compile(string csharpCode)
        {
        ...
        public int SetMagicFunction(ref string csharpCode, int relative_position, string nameSpace, string ClassName, string methodName, List<ViewData> viewDatas, Config config)
        {
                ...
                for (int i = 0; i < viewDatas.Count; i++)
                {
                    ...
                    if (styles.Length > 0)
                    {
                        ...
                        for(int j=0;j<styles.Length;j++)
                        {
                            ...
                            {
                                NFinal.Template.ASPX aspx = new Template.ASPX();
                                sr = new StreamReader(tplFileName, System.Text.Encoding.UTF8);
                                compressWriter = new StringWriter();
                                compressCode = Template.HtmlCompressor.compress(sr.ReadToEnd());
                                if (IsVarName)
                                {
                                    sharpCodes[j] = aspx.Render(compressWriter, compressCode, 5);
                                }
                                else
                                {
                                    sharpCodes[j] = aspx.Render(compressWriter, compressCode, 3);
                                }
                                sr.Close();
                            }
                            else
